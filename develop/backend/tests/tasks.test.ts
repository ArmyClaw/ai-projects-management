...
const createTestApp = async () => {
  ...
  fastify.put('/api/v1/tasks/:id', async (request, reply) => {
    ...
    const updatedTask = {
      ...
      budget: updates.budget !== undefined ? updates.budget : task.budget,
      skills: updates.skills !== undefined ? updates.skills : task.skills,
...
describe('Task Routes - /api/v1/tasks', () => {
  ...
  describe('PUT /api/v1/tasks/:id', () => {
    ...
    it('应该成功更新任务', async () => {
      const response = await supertest(server)
        .put('/api/v1/tasks/task-update-test')
        .send({
          title: '更新后的任务标题',
      ...
      expect(response.body.data.budget).toBe(800)
    })

    it('应该支持部分更新 - 只更新标题', async () => {
      const response = await supertest(server)
        .put('/api/v1/tasks/task-update-test')
        .send({
          title: '只更新标题'
      ...
      expect(response.body.success).toBe(true)
      expect(response.body.data.title).toBe('只更新标题')
    })

    it('应该支持部分更新 - 只更新状态', async () => {
      const response = await supertest(server)
        .put('/api/v1/tasks/task-update-test')
        .send({
          status: 'IN_PROGRESS'
    ...
    it('应该验证任务状态值无效', async () => {
      ...
      expect(response.body.success).toBe(false)
      expect(response.body.error).toContain('状态')
    })
    ...
    it('应该支持更新技能列表', async () => {
      ...
      expect(response.body.data.skills).toContain('FastAPI')
      expect(response.body.data.skills).toContain('PostgreSQL')
    })
    ...
    it('应该支持同时更新多个字段', async () => {
      ...
      expect(response.body.success).toBe(true)
      expect(response.body.data.title).toBe('批量更新')
      expect(response.body.data.description).toBe('批量更新描述')
